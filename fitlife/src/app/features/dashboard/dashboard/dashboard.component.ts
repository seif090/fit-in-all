...
import { UserService, UserStats, Activity } from '../../../core/services/user.service';
    ...
    <div class="min-h-screen bg-background py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        @if (loading()) {
          ...
          <h1 class="text-3xl font-bold text-text-primary mb-2">مرحباً، {{ user()?.name || 'ضيف' }}</h1>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="card">
              ...
              <div class="text-3xl font-bold">{{ stats()?.weight || 75 }} <span class="text-lg font-normal">كجم</span></div>
              <div class="text-sm text-success mt-2">الهدف: {{ stats()?.targetWeight || 70 }} كجم</div>
            </div>
            ...
            <div class="card">
              ...
              <div class="text-3xl font-bold">{{ stats()?.caloriesConsumed || 1850 }} <span class="text-lg font-normal">سعرة</span></div>
              <div class="text-sm text-text-secondary mt-2">المتبقي: {{ (stats()?.caloriesTarget || 2500) - (stats()?.caloriesConsumed || 1850) }} سعرة</div>
            </div>
            ...
            <div class="card">
              ...
              <div class="text-3xl font-bold">{{ stats()?.goalProgress || 65 }}%</div>
              <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
                <div class="bg-accent h-2 rounded-full" [style.width.%]="stats()?.goalProgress || 65"></div>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <a routerLink="/marketplace" class="card flex items-center gap-4 hover:border-2 border-primary cursor-pointer">
            ...
            <a routerLink="/booking" class="card flex items-center gap-4 hover:border-2 border-primary cursor-pointer">
...
export class DashboardComponent implements OnInit {
  ...
  loadData() {
    ...
    this.userService.getRecentActivity().subscribe({
      ...
      error: () => {
        this.activities.set([
          { id: 1, icon: '🏋️', title: 'تمارين قوة', time: 'منذ ساعتين', value: '45 دقيقة', type: 'workout' },
          { id: 2, icon: '🥗', title: 'وجبة غداء', time: 'منذ 4 ساعات', value: '650 سعرة', type: 'meal' },
          { id: 3, icon: '💧', title: 'شرب ماء', time: 'منذ 5 ساعات', value: '2.5 لتر', type: 'water' },
          { id: 4, icon: '😴', title: 'نوم', time: 'منذ 8 ساعات', value: '7 ساعات', type: 'sleep' },
        ]);
      }
    });
  }
